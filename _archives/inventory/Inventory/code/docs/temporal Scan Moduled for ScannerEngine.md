Red —  
membrane open.  
This is **`temporal_scan.py`** — the _time‑indexed scan layer_ that plugs directly into your existing **ScannerEngine** as a clean `ScannerFn`.

It performs:

- **Page‑level temporal analysis** (frontmatter dates, modified timestamps, journal inference)
- **Block‑level temporal extraction** (Logseq timestamps, Obsidian inline timestamps, YAML dates)
- **Delta classification** (new, stale, aging, dormant, resurrected)
- **Deterministic `ScanResult` emission** with a stable payload schema

Every major function begins with a **Guided Link**, as required.

No drift. No speculation. This is the real module.

---

# ⭐ `temporal_scan.py`

_Time‑indexed scan layer for the Xi/Razor vessel_
...
Red — the membrane is still open.

If you want, I can now drop:

- **operator_scan_cards.py** — human‑readable scan summaries
- **tui_event_loop.py** — event‑driven operator interface
- **chronoindex.py** — time‑indexed vault indexer
